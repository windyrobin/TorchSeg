...
class HostDeviceMem(object):
    def __init__(self, host_mem, device_mem, host_mem_back):
        self.host = host_mem
        self.device = device_mem
        self.host_back = host_mem_back
...
def allocate_buffers(engine):
    ...
    for binding in engine:
        ...
        print('nptype:', engine.get_binding_dtype(binding))
        ...
        host_mem_back = cuda.pagelocked_empty(size, dtype)
        device_mem = cuda.mem_alloc(host_mem.nbytes)
        # Append the device buffer to device bindings.
        bindings.append(int(device_mem))
        # Append to the appropriate list.
        if engine.binding_is_input(binding):
            inputs.append(HostDeviceMem(host_mem, device_mem, host_mem_back))
        else:
            outputs.append(HostDeviceMem(host_mem, device_mem, host_mem_back))
...
def do_inference_sync(context, bindings, inputs, outputs, stream, batch_size=1):
    # Transfer input data to the GPU.
    [cuda.memcpy_htod(inp.device, inp.host) for inp in inputs]
    # Run inference.
    ret = context.execute(batch_size=batch_size, bindings=bindings)
    print('execute ret:', ret)
    # Transfer predictions back from the GPU.
    [cuda.memcpy_dtoh(out.host, out.device) for out in outputs]
    [cuda.memcpy_dtoh(inp.host_back, inp.device) for inp in inputs]
    # Synchronize the stream
    #stream.synchronize()
    # Return only the host outputs.
    return [out.host for out in outputs]
